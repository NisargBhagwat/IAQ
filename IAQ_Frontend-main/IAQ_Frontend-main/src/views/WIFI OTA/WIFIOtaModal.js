...
import { fetchAsyncCreateWifiOta } from "../../Slices/WifiOtaSlice";

const WIFIOtaModal = ({ show, handleClose }) => {
  const initialValues = {
    otaVersion: "",
    otaUrl: "",
    wifiStatus: false,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const wifistatus = values.wifiStatus ? true : false;
        const data = {
          otaVersion: values.otaVersion,
          otaUrl: values.otaUrl,
          status: wifistatus,
        };
        dispatch(fetchAsyncCreateWifiOta(data));
        handleClose();
        resetForm();
      } catch (error) {
